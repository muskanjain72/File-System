...
void read_file(Client *c, const char *filename, int fd)
{
    // Basic sanity checks
    if (!filename) {
        printf("read_file: filename is NULL\n");
        return;
    }

    // Step 1: Resolve file (supports folder/filename paths like "docs/file")
    printf("Looking up file: %s\n", filename);
    nameserver_log("READ_REQUEST user=%s id=%d ip=%s port=%d file=%s",
    ...
    if (!file)
    {
        ...
        nameserver_log("READ_FAIL not_found user=%s file=%s", c->client_name, filename ? filename : "(null)");
    ...
    if (!has_permission) {
        ...
        nameserver_log("READ_DENIED user=%s id=%d file=%s", c->client_name, c->client_id, filename);
        snprintf(outp.msg, sizeof(outp.msg), "ERR|no_permission|%s", filename);
        send(fd, &outp, sizeof(outp), 0);
        return;
    }
    snprintf(outp.msg, sizeof(outp.msg), "READ|%s|%s|%d|%d",
             file->filename,
             ss->ip_address,
             ss->port,
             file->inode_no);
    printf("Prepared read response: %s\n", outp.msg);
    nameserver_log("READ_RESPONSE user=%s id=%d file=%s ss=%s port=%d inode=%d",
                   c->client_name, c->client_id, file->filename, ss->ip_address, ss->port, file->inode_no);
    file->rtime = time(NULL);
    printf("Updated read time for file '%s' to %ld\n", filename, file->rtime);
    ...
    nameserver_log("READ_SENT_TO_CLIENT id=%d user=%s msg=\"%s\"", c->client_id, c->client_name, outp.msg);
}
