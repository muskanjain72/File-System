...
void revert(Client *c, const char *filename, const char* tag, Packet *outp, int fd) {
    ...
    nameserver_log("REVERT_REQUEST user=%s id=%d file=%s", c->client_name, c->client_id, filename);

    FileMeta *file = find_file_by_path(filename);
    if (!file) {
        snprintf(outp->msg, sizeof(outp->msg), "ERR|not_found|%s", filename);
        nameserver_log("REVERT_FAIL not_found user=%s file=%s", c->client_name, filename);
        return;
    }
    ...
    if (!valid) {
        ...
        nameserver_log("REVERT_FAIL stale_pointer user=%s file=%s", c->client_name, filename);
        return;
    }
    ...
    if (!has_permission) {
        ...
        nameserver_log("REVERT_DENIED user=%s file=%s", c->client_name, filename);
        return;
    }
    ...
    while(cp){
        if(strcmp(cp->tag, tag)==0){
            ...
            if (ss_id < 0 || ss_id >= MAX_STORAGE || !storages[ss_id]) {
                ...
                nameserver_log("REVERT_FAIL ss_unavailable user=%s file=%s ss=%d", c->client_name, filename, ss_id);
                return;
            }
            ...
            snprintf(outp->msg, sizeof(outp->msg), "REVERT|%s|%d|%s", ss->ip_address, ss->port, destpath);
            nameserver_log("REVERT_OK user=%s file=%s tag=%s", c->client_name, filename, tag);
            return;
        }
        cp = cp->next;
    }
