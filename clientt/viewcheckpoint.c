...
void view_check(const char* ip_address, int port, const char* destpath) {
    ...
    // Send read checkpoint request
    char req[512];
    snprintf(req, sizeof(req), "READ_CHECKPOINT %s", destpath);
    if (send(sock, req, strlen(req), 0) <= 0) {
        perror("send read checkpoint request");
        close(sock);
        return;
    }

    printf("Sent read checkpoint request for %s\n", destpath);

    // Receive file data
    char buffer[1024];
    ssize_t n;
    printf("Checkpoint contents:\n");
